...
class _EditCropPageState extends State<EditCropPage> {
  ...
  Future<void> saveNewValue(String field, String newValue) async {
    ...
    QuerySnapshot querySnapshot = await userCollection.where('id', isEqualTo: "aLfB0xeahWG4tWKZ5ibB").get();
  ...
  void update() {
    _timer = Timer.periodic(Duration(seconds: 10), (timer) {
      setState(() {
        _initial += 0.04;
        trigerNotification();
        print(_initial); // Print the updated value inside the callback
      });
    });
  }
          ...
          body: Container(
            child: Column(
              children: [
                ...
                Text(
                  '${ctrl.listingsShowInUi[widget.index].name ?? 'No name'}',
                                      ...
                                      child: Row(
                                        children: [
                                          ...
                                          IconButton(
                                            ...
                                            onPressed: () => editField('plant_date'),
                                          ),
                                        ],
                                      ),
                                    ),
